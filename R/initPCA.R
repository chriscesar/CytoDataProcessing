...
# arrow::write_parquet(x=df0,sink="tmp/parquet.data")
...
library(GGally)
#ggpairs(na.omit(dfnorm))

plot(pca01)
# factoextra::fviz_pca(pca01)

M <- cor(na.omit(dfnorm))
corrplot::corrplot(M, method="color",type="lower")
corrplot::corrplot(M, type="upper", order="hclust",method="color",
                   col=brewer.pal(n=8, name="RdYlBu"),
                   tl.col="black", tl.srt=90, tl.cex = .6)
